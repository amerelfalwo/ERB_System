...
async def create_supplier_advance_payment(
    supplier_id: int,
    data: dict,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """Record an advance/prepayment to a supplier regardless of current balance."""
    party = db.execute(
        select(Party).where(
            Party.id == supplier_id,
            Party.tenant_id == current_user.tenant_id,
    ...
    await delete_cache(current_user.tenant_id if current_user else 0, f"supplier:{supplier_id}:summary")
    await invalidate_tenant_cache(current_user.tenant_id, ["dashboard", "reports", "parties", "suppliers", "supplier"])
